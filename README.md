...
**现状（v0.4.1）**：腾讯云 VPS ✅ · 异步提交+归档 ✅ · 对抗安全热修（basicauth / 密钥不进浏览器 / URL 白名单）✅
...
团队浏览器 ──HTTPS──► Caddy（仅 80/443）
                        ├ api / collab / submit / dash / s3 / minio
Docker:
...
5. 按部署指南配置 New-API 渠道（Base URL=`http://seedance-adapter:18008` + Bearer）与 NocoDB 建表
6. **安全**：`caddy hash-password` 写入 Caddyfile；确认 submit/dash 401；勿公网暴露 MinIO 控制台
